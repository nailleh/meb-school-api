...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        for (const [key, info] of Object.entries(personelLinks)) {
            ...
            const isYonetici = hasYoneticiKeyword && (!hasOgretmenKeyword || 
                              (hasOgretmenKeyword && (titleUpper.includes('MÜDÜR') || titleUpper.includes('BAŞKAN'))));

            if (!isYonetici) continue;
        ...
        return res.json({
            success: true,
            yonetici_sayisi: yoneticiler.length,
